...
func TestParseHTTPLogJSONExit(t *testing.T) {
	sig, ok := ParseHTTPLog(`{"ts":"2026-09-03T17:49:57.551+00:00","level":"info","event":"exit","service":"agendamentoapi","status":200,"durationMs":486}`)
	if !ok {
		t.Fatal("expected parse ok")
	}
	if sig.Service != "agendamentoapi" || sig.Status != 200 || sig.DurationMs != 486 {
...
func TestParseHTTPLogJSONExitSkipsEntry(t *testing.T) {
	_, ok := ParseHTTPLog(`{"event":"entry","service":"agendamentoapi","msg":"http.request"}`)
	if ok {
		t.Fatal("entry should not produce metrics")
	}
}

func TestParseHTTPLogVenuzResponseLine(t *testing.T) {
	line := `260903/174932.161, (1788457772161:b670754c7016:1:mtlko8vi:11223) [response,api] post /agendamento/filtrar {"sortBy":"agendamentoInicio"} 200 (486ms)`
...
func TestParseHTTPLogErrorStatus(t *testing.T) {
	sig, ok := ParseHTTPLog(`{"event":"exit","service":"pagamentosapi","status":500,"durationMs":120}`)
